//Original file location : frappe/frappe/desk/page/desktop/desktop.js
...
$.extend(frappe.desktop_utils, {
	...
	create_desktop_modal: function (icon, icon_title, icons_data, grid) {
		if (!this.modal) {
			this.modal = new CustomDesktopModal(icon);
		}
...
frappe.pages["desktop"].on_page_load = function (wrapper) {
	...
	let desktop_page = new CustomDesktopPage(page);
...
class CustomDesktopPage {	
	...
	make() {
		...
		this.icon_grid = new CustomDesktopIconGrid({
	...
	start_editing_layout() {
		this.edit_mode = true;
		const me = this;
		this.desktop_pane = new CustomIconsPane();
...
class CustomDesktopIconGrid {
	...
	add_folder() {
		CustomDesktopIconGrid.folder_count++;
		let icon = frappe.model.get_new_doc("Desktop Icon");
		icon.icon_type = "Folder";
		icon.label = `Untitled ${CustomDesktopIconGrid.folder_count}`;
		icon.idx = 100000;
		frappe.new_desktop_icons.push(icon);
		frappe.new_icons.push(icon);
		return icon;
	...
	make_icons(icons_data, grid) {
		icons_data.forEach((icon) => {
			let icon_obj = new CustomDesktopIcon(icon, this.in_folder, this);
...
class CustomDesktopIcon {
	...
	setup_click() {
		...
		if (this.child_icons?.length && (this.icon_type == "App" || this.icon_type == "Folder")) {
			$(this.icon).on("click", () => {
				...
				let title = new CustomInlineEditor($title, this.icon_data.label, function (
					old_value,
					new_value
				) {
	...
	render_folder_thumbnail() {
		if (this.icon_type == "Folder") {
			...
			this.folder_grid = new CustomDesktopIconGrid({
...
class CustomDesktopModal {
	...
	setup(icon_title, child_icons_data, grid_row_size) {
		...
		this.child_icon_grid = new CustomDesktopIconGrid({
...
class CustomIconsPane {
	...
	show() {
		...
		this.grid = new CustomDesktopIconGrid({
